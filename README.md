...
插件安装并重载后，在 WebUI 的「插件」→「已安装」页面，找到本插件，点击“配置”按钮进行设置：

| 配置项 | 说明 | 必填 |
| :--- | :--- | :---: |
| `zssq_api_base` | 追书神器 API 服务器根地址，默认无需修改 | 否 |
| `geetest_appkey` | 极验验证码自动过验服务的 AppKey | 是 |
...
| `admin_whitelist` | 管理员用户 ID 列表，多个用英文逗号 `,` 分隔 | 否 |

> 青龙面板需开启 OpenAPI 功能，并在「开放平台」→「应用」中创建应用以获取 `Client ID` 和 `Client Secret`。
